class Worker::ResumeEducationsController < ApplicationController
  ...
  def destroy
    begin
      object = (AccountResumeEducation.by_account_id(current_account.id).find(params[:id]) rescue raise "内容不存在")
      object.destroy
    rescue Exception => ex
      flash[:error] = ex.message
